...
#define Podmiot_WiFi "."
#define Podmiot_Pilot "r"
#define Podmiot_Przycisk "p"
...
void Otworz(Akcja akcja)
{
	switch (akcja)
	{
		case On:
		{
			...
			{
				Zamknij(Akcja(Off));
				if (digitalRead(Krancowka_Otwarcia) == LOW || digitalRead(Przerwanie_Wiazki) == LOW )  // jak jest aktywna krancowka
				{
					return;
				}
				delay(sleep);
...
void Zamknij(Akcja akcja)
{
	switch (akcja)
	{
		case On:
		{
			...
			{
				Otworz(Akcja(Off));
				if (digitalRead(Krancowka_Zamkniecia) == LOW || digitalRead(Przerwanie_Wiazki) == LOW )  // jak jest aktywna krancowka
				{
					return;
				}
				delay(sleep);
...
void loop() 
{
	...
	if (!digitalRead(Krancowka_Otwarcia && !digitalRead(Krancowka_Zamkniecia)))
	{
		changed = true;
	}
...
//------------------------------WIFI----------------------------------
	...
	if(changed)// potwierdzenia n wifi
	{
		...
		if (!digitalRead(Krancowka_Otwarcia) && !digitalRead(Krancowka_Zamkniecia))
		{
			pisz("Awaria! Aktywne dwie krancowki");
			esp.print(Podmiot + "g");
		}
		else if(Otworz())
		{
		...
		else if(!digitalRead(Krancowka_Otwarcia))
		{
			esp.print(Podmiot + "c");
		}
		else if(!digitalRead(Krancowka_Zamkniecia))
		{
			esp.print(Podmiot + "d");
		}
		else if(!digitalRead(Przerwanie_Wiazki))
		{
			esp.print(Podmiot + "e");
		}
