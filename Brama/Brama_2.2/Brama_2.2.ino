...
bool ipd[5];  // numery połączeń ipd
...
if(esp.available() > 0)
{
  delay(100); // czas na kompletny odbior danych
  int a = esp.available();
  int i=0;
  for(i; i <a;i++)
  {
     bufor[i] = esp.read();
     pisz(String(char(bufor[i]))); // pisz(String(bufor[i]));
     //pisz("\n");
  }
  ...
  if(bufor[i+1] == 44 && bufor[i+2] == 67 &&bufor[i+3] == 79 && bufor[i+4] == 78 && bufor[i+5] == 78 && bufor[i+6] == 69 && bufor[i+7] == 67 && bufor[i+8] == 84)  //
  {
    if(bufor[i+10] == 70 && bufor[i+11] == 65 && bufor[i+12] == 73 && bufor[i+13] == 76)// gdzy ,connect fail
    {
      ipd[(bufor[i]) -48 ] = false;
      
      pisz("connct fail na ipd, ");
      pisz(String(bufor[i] - 48));
      pisz("\n");
    }
    else // jestli tylko ,connect
    {
    ipd[(bufor[i]) -48 ] = true;
  ...
  if(bufor[i+1] == 44 && bufor[i+2] == 67 &&bufor[i+3] == 76 && bufor[i+4] == 79 && bufor[i+5] == 83 && bufor[i+6] == 69 && bufor[i+7] == 68) //,closed
  {
